...
The Paysecure Webhooks API notifies merchants about transaction status updates.
...
- `paysecure_sign` (string) - Signature for payload verification.

<Warning>Authorization requires verifying the `paysecure_sign` header using your public key.</Warning>
...
```json JSON
{
  ...
  "status": "paid",
  "brandId": "7890",
  "paysecure_sign": "BASE64_SIGNATURE"
}
```
...
- **Verify `paysecure_sign` to ensure authenticity.**
- **Handle response codes appropriately.**

Integrate Paysecure webhooks to stay updated with transaction statuses. 🚀