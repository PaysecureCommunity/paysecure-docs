---
title: "Paysecure PayOut API"
description: "API for initiating payouts, checking transaction status, and retrieving balance information using Paysecure."
...
(async () => {
  ...
  const response = await axios.get(
    `https://staging.paysecure.net/api/v1/getpayout/${payoutId}`,
    {
      headers: {
        "Authorization": "Bearer <your_token>",
        "Content-Type": "application/json"
...
(async () => {
  const response = await axios.post(
    "https://staging.paysecure.net/api/v1/getBalance",
