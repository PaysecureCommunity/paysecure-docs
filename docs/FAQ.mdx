...
id: FAQ
title: FAQ
description: Frequently asked questions about the Fundstream API.
---
...
## Q7. Does Fundstream support card in sandbox mode?
**Ans:** Yes, Fundstream supports card transactions in sandbox mode for testing purposes.
...
- **For Pay-In:**
  ```
  POST https://api.fundstream.net/api/v1/purchases/
  ```
- **For Pay-Out:**
  ```
  POST https://api.fundstream.net/api/v1/payout/
  ```
...
- **To check the balance after a Pay-Out transaction:**
  ```
  GET https://api.fundstream.net/api/v1/getpayout/{payoutId}/
  ```
